var pokemon = ["abra", "bulbasaur", "charmander", "diglet", "eevee", "fennekin", "geodude", "hitmonlee", "ivysaur", "jigglypuff", "koffee", "lapras", "mr. mime", "nidoran", "onix", "pikachu", "snorlax", "mew", "ditto", "squirtle", "dragonite", "zapdos", "meowth", "psyduck", "gengar", "vaporeon", "charizard", "blastoise", "venusaur", "pidgeot", "togepi"];
console.log(pokemon);
...
$(document).on('click', 'button', function () {
    ...
    $.ajax({
        ...
        for (var j = 0; j < 10; j++) {
            ...
            $("img").on("click", function () {
                ...
                if ($(this).attr('src') == response.data[0].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[0].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[1].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[1].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[2].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[2].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[3].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[3].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[4].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[4].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[4].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[4].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[5].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[5].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[6].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[6].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[7].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[7].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[8].images.fixed_height_small_still.url) {
                    $(this).attr('src', response.data[8].images.fixed_height_small.url)
                }
                if ($(this).attr('src') == response.data[9].images.fixed_height_small_still.url) {
    ...
    $.ajax({
        ...
        $('.col-3').append("Attacks: ", attacks1, attacks2, attacks3, attacks4, breakline);
